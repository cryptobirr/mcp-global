...
echo "[1/11] Checking for Architecture overview..."
...
echo "[2/11] Checking for similar patterns..."
if grep -q "## Similar Patterns Found\|## Similar Patterns\|# Similar Patterns" "$RESEARCH_FILE"; then
    PATTERN_COUNT=$(grep -c "### Pattern [0-9]\+:\|### Pattern:" "$RESEARCH_FILE" || echo 0)
...
echo "[3/11] Checking for integration points..."
...
echo "[4/11] Checking for testing infrastructure..."
...
echo "[5/11] Checking for risks and constraints..."
...
echo "[6/11] Checking for blocking decisions section..."
...
echo "[7/11] Checking blocking decision options..."
BLOCKING_COUNT=$(grep -c "🚨\|BLOCKING DECISION" "$RESEARCH_FILE" || echo 0)
if [ "$BLOCKING_COUNT" -eq 0 ]; then
...
echo "[8/11] Checking blocking decision recommendations..."
if [ "$BLOCKING_COUNT" -eq 0 ]; then
    check_pass "No blocking decisions to validate"
else
...
echo "[9/11] Checking file location..."
if [[ "$RESEARCH_FILE" =~ \.dev/issues/[0-9]+-[^/]+/02-research/research\.md$ ]] || [[ "$RESEARCH_FILE" =~ \.dev/issues/[0-9]+-[^/]+/02-research/research-v[0-9]+\.md$ ]]; then
...
echo "[10/11] Checking for Impact Analysis & Regression Risks..."
if grep -q "## Impact Analysis\|# Impact Analysis" "$RESEARCH_FILE"; then
    # Check for Affected Features section
    if grep -q "### Affected Features\|## Affected Features" "$RESEARCH_FILE"; then
        # Check for Regression Test Coverage Matrix
        if grep -q "### Regression Test Coverage Matrix\|## Regression Test Coverage Matrix" "$RESEARCH_FILE"; then
            # Check for Blast Radius Summary
            if grep -q "### Blast Radius Summary\|## Blast Radius Summary" "$RESEARCH_FILE"; then
...
# Check 11: Exactly one phase label on GitHub issue
echo "[11/11] Checking GitHub issue has exactly one phase label..."
# Extract issue number from research file path
ISSUE_NUM=$(echo "$RESEARCH_FILE" | grep -oE '/[0-9]+-' | grep -oE '[0-9]+' | head -1)

if [ -n "$ISSUE_NUM" ]; then
    PHASE_COUNT=$(gh issue view "$ISSUE_NUM" --json labels --jq '.labels[] | select(.name | startswith("phase:")) | .name' 2>/dev/null | wc -l | tr -d ' ')

    if [ "$PHASE_COUNT" -eq 1 ]; then
        CURRENT_PHASE=$(gh issue view "$ISSUE_NUM" --json labels --jq '.labels[] | select(.name | startswith("phase:")) | .name')
        check_pass "Issue #$ISSUE_NUM has exactly 1 phase label: $CURRENT_PHASE"
    elif [ "$PHASE_COUNT" -eq 0 ]; then
        check_fail "Issue #$ISSUE_NUM has NO phase labels" \
            "Every issue must have exactly one phase: label"
    else
        PHASE_LABELS=$(gh issue view "$ISSUE_NUM" --json labels --jq '.labels[] | select(.name | startswith("phase:")) | .name' | tr '\n' ', ')
        check_fail "Issue #$ISSUE_NUM has $PHASE_COUNT phase labels" \
            "Found: $PHASE_LABELS - Must have exactly ONE phase label"
    fi
else
    check_fail "Could not extract issue number from path" \
        "Expected path format: .dev/issues/NNN-slug/02-research/research.md"
fi
echo ""
