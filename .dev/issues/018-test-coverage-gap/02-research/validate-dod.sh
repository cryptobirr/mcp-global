...
if grep -q "## Similar Patterns Found\|## Similar Patterns\|# Similar Patterns" "$RESEARCH_FILE"; then
    PATTERN_COUNT=$(grep -c "### Pattern:" "$RESEARCH_FILE" || echo 0)
...
if [[ "$RESEARCH_FILE" =~ \.dev/issues/[0-9]+-[^/]+/02-research/research\.md$ ]]; then
...
ISSUE_NUM=$(echo "$RESEARCH_FILE" | grep -oE '/[0-9]{3}-' | grep -oE '[0-9]+')
...
# Calculate DoD score
TOTAL_CHECKS=11
DOD_SCORE=$(( (PASS_COUNT * 100) / TOTAL_CHECKS ))
