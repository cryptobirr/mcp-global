...
if grep -q "## Similar Patterns Found\|## Similar Patterns\|# Similar Patterns" "$RESEARCH_FILE"; then
    ...
    elif grep -qi "no patterns exist\|greenfield.*no existing\|NOT FOUND in codebase" "$RESEARCH_FILE"; then
        check_pass "Greenfield/new pattern - explicitly stated no patterns exist"
    else
