...
if grep -q "## Similar Patterns Found\|## Similar Patterns\|# Similar Patterns" "$RESEARCH_FILE"; then
    ...
    elif grep -qi "no patterns exist\|greenfield.*no existing\|NOT FOUND\|will be the.*FIRST" "$RESEARCH_FILE"; then
        check_pass "Greenfield project - explicitly stated no/limited patterns exist"
    else
