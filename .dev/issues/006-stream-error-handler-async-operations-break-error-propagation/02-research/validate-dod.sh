...
if grep -q "## Similar Patterns Found\|## Similar Patterns\|# Similar Patterns" "$RESEARCH_FILE"; then
    PATTERN_COUNT=$(grep -c "### Pattern [0-9]:\|### [0-9]\+\. Pattern:" "$RESEARCH_FILE" || echo 0)
...
if grep -q "## Blocking Decisions\|# Blocking Decisions" "$RESEARCH_FILE"; then
    BLOCKING_COUNT=$(grep -c "🚨\|### 🚨 Decision Required:" "$RESEARCH_FILE" || echo 0)
...
if [ "$BLOCKING_COUNT" -eq 0 ]; then
    ...
    OPTIONS_COUNT=$(grep -c "#### Option [A-Z]:\|**Option [A-Z]:\|### Option [A-Z]:" "$RESEARCH_FILE" || echo 0)
